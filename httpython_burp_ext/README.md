...
- 各リクエストは `def <func>(base_url, <引数...>):` の関数となり、
  一覧の並び順で出力される。
- コメントを入力した関数のみ、先頭に 1 行コメントが付く（未入力ならコメント行は
  出力しない）。日本語コメントも文字化けせずに生成される。
...
- **Send to httpython の二重取り込み対策**：`getSelectedMessages()` は request と
  response を内包しており、Repeater 等では同一メッセージが request 側・response 側の
  別オブジェクトとして2回返ることがある。この重複返却やメニューのアクションの
  二重発火があっても、リクエスト内容で照合して同一操作内では同じリクエストを
  1件だけ取り込む。意図して同じ内容を後から再度送る操作は妨げない。
